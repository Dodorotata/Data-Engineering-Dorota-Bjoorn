...
import requests, pytz                                   # request to get data (jmf curl i dash), pytz to get timezone
...
def _transform_stockholm_timezone(task_instance):
    data = task_instance.xcom_pull(task_ids = "extract_liseberg.extract_queue_time")        # task_ids = "task_group.task_id" ; xcom_pull to get data from db
    for ride in data:
...
def extract_queue_time():
    extract_queue_time = PythonOperator(
        ...
        op_args = [theme_parks["asterix"]],
        do_xcom_push = True                     # data pushed to xcom, transformed by def _transform_sto... and pulled by task i load
    )
